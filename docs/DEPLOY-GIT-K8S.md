...
5. **`kubectl get pods`** and a best-effort **`ollama pull mistral`**
...
4. **deploy_self_hosted** – If **`DEPLOY_MODE=self_hosted`**:
  ...
  - Optionally runs a one-time `ollama pull mistral` in the cluster (non-blocking).
...
- **Ollama**: The workflow tries to pull `mistral` once. You can pull other models manually:
  ```bash
