...
| `CI_FORCE_BUILD_ALL` | Variable | `true` | When **true** / **1** / **yes**, each **push to `main`** runs **all eight** image builds and the full backend pytest matrix (same idea as **Actions → Build and Deploy → Force rebuild all**). **Turn off or delete after** the run you care about, or every push will pay full Docker cost. **Alternative (no variable):** put **`[ci-full-build]`** in the commit message and push to `main` for a **one-off** full matrix. |
