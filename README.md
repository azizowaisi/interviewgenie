...
**CPU vs container images:** CI builds **multi-arch** (`linux/amd64` + `linux/arm64`) by default; deploy applies a **Docker Hub pull secret** for in-cluster pulls. See **`docs/ORACLE-ARCHITECTURE.md`** for **502** / **ImagePullBackOff** troubleshooting.
...
**Production Docker images — use `main`, not the PR workflow.** After you merge, **Build and Deploy** pushes immutable tags **`sha-<merge commit>`** to Docker Hub; **that** is what deploy (`kubectl set image` in `scripts/ci/k8s-apply.sh`) pins the cluster to. The **CI** workflow on pull requests may also push images tagged **`pr-<#>-<headSha>`** for validation and cache warming, but those tags **do not** match the merge commit and **must not** be treated as what you run in production. **Always ship from `main`** (merge → Build and Deploy, or run that workflow manually on `main`).
...
- **Deploy**: **`DEPLOY_MODE`** is read from the **repository variable** *or* (if empty) the **repository secret** of the same name — variable wins. Then: **`KUBE_CONFIG`** → remote **`kubectl apply`**; else **SSH secrets** → **SSH bootstrap**. **`DEPLOY_MODE=ssh`** forces SSH even with `KUBE_CONFIG`; **`none`** / **`off`** = push only. One-shot: `./scripts/deploy/apply-k8s-from-repo.sh` with `KUBECONFIG` set. See [Deploy through Git to Kubernetes (single VM)](docs/DEPLOY-GIT-K8S.md).
